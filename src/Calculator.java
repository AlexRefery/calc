...
public class Calculator {
    ...
    public static String calc(String input) {
        String[] operands = input.split(OPERATION_REGEX);
        String operator = String.valueOf(input.replace(operands[0], "").charAt(0));
        if (operands.length != 2 || operands[0].isBlank()) {
            throw new IllegalArgumentException("Обязательны два положительных аргумента!");
        }
        if (input.length() - input.replaceAll(OPERATION_REGEX, "").length() > 1) {
        ...
        return isRoman(operands[0]) ? converterArabToRoman(result) : Integer.toString(result);
    }
}